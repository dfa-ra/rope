...
- **0.3.51:** composer and search can use an incognito keyboard from Settings. Security: LocalStore `decryptBytes` and IdentityVault wrap require a 12-byte AES-GCM IV; `SecretKv.unwrap` returns null on a corrupt wrap; GitHub APK asset names reject CR/LF; APKs install only from `cache/updates`. Not envelope crypto.
