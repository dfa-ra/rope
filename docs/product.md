...
- **0.3.51 security:** LocalStore `decryptBytes` and IdentityVault wrap require a 12-byte AES-GCM IV; `SecretKv.unwrap` returns null on a corrupt wrap; GitHub APK asset names reject CR/LF; APKs install only from `cache/updates`. Also: Settings in-app sound for incoming heads-up while the app is open. Not envelope crypto.
