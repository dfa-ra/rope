...
- **0.3.51 UI:** Settings «Говорить, поднеся к уху» records a voice note when the phone is raised to the ear in an open chat; lowering sends. Distinct from playback pause and from call earpiece. Default on (`raise_speak` != `"0"`). LocalStore stays v6.
