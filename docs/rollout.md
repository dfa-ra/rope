...
На экране **Server → Обновить приложение**. APK ставится поверх, без удаления. Нужен `versionCode` больше текущего и та же подпись (debug-релизы обновляют debug). История и ключи остаются.
