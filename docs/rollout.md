...
Installer заменяет `/opt/rope/bin/rope-server`, ставит/обновляет coturn на том же IP, **всегда** пишет `public_host` / `turn_secret` / порты в config (секрет создаётся, если его не было), слушает `0.0.0.0`, ставит `external-ip`, **перезапускает и rope, и coturn**, **не** трогает `data.db` и TLS. Если 443 занят (Caddy/nginx) — TURNS на 5349 и именно этот порт попадает в `GET /v1/info`. Повторный запуск на уже установленном Rope — то же самое (не пишет «на сервере уже есть Rope»). После обновления ядра звонки берут ICE с `GET /v1/info`. Карточка TURN в приложении показывает, слушает ли coturn, а не только «секрет прописан».
