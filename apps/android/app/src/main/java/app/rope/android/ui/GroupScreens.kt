...
fun GroupInfoPane(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (canDelete) {
                if (confirmDelete) {
                    GlowButton(GroupDeleteRules.CONFIRM, onDeleteGroup, Modifier.fillMaxWidth())
                    TextButton(onClick = { confirmDelete = false }, modifier = Modifier.fillMaxWidth()) {
                        Text(GroupDeleteRules.CANCEL)
                    }
                } else {
                    QuietButton(GroupDeleteRules.LABEL, { confirmDelete = true }, Modifier.fillMaxWidth())
                }
            }
        }
    }
