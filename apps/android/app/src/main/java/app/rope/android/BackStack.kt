...
object BackStack {
    ...
    fun listForForward(stack: List<Screen>): List<Screen> {
        val trimmed = stack.dropLastWhile {
            it == Screen.Chat || it == Screen.GroupInfo || it == Screen.NewGroup ||
                it == Screen.PeerProfile || it == Screen.Archive
        }
    ...
    fun decide(state: UiState, hints: OverlayHints = OverlayHints()): BackLayer = when {
        ...
        composerLive(state.screen) && (state.recordingVideoNote || (state.recording && !state.recordingLocked)) -> BackLayer.CancelRecording
