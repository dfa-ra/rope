...
class RopeRepository(private val app: Application) {
    ...
    fun setChatsDmOnly(dmOnly: Boolean) {
        if (_state.value.chatsDmOnly == dmOnly) return
        store.saveChatsDmOnly(dmOnly)
        _state.value = _state.value.copy(chatsDmOnly = dmOnly)
    }
