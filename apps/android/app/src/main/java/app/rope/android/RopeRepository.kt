...
data class UiState(
    ...
    val callsMissedOnly: Boolean = false,
...
class RopeRepository(private val app: Application) {
        ...
        scope.launch {
            try {
                ...
                _state.value = _state.value.copy(
                    ...
                    callsMissedOnly = store.callsMissedOnly(),
                )
    ...
    fun setCallsMissedOnly(missedOnly: Boolean) {
        if (_state.value.callsMissedOnly == missedOnly) return
        store.saveCallsMissedOnly(missedOnly)
        _state.value = _state.value.copy(callsMissedOnly = missedOnly)
    }
