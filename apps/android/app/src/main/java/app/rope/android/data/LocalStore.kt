...
class LocalStore(context: Context) : SQLiteOpenHelper(context, "rope-local.db", null, VERSION) {
    ...
    fun saveHeardOnceVoice(ids: Set<String>) {
        put(VoiceOnceRules.KEY, VoiceOnceRules.putHeard(ids))
    }

    fun heardOnceVoice(): Set<String> = VoiceOnceRules.heardIds(get(VoiceOnceRules.KEY))
