...
class LocalStore(context: Context) : SQLiteOpenHelper(context, "rope-local.db", null, VERSION) {
    ...
    fun saveRejectIncomingCalls(enabled: Boolean) {
        put(PrivacyCallsRules.KEY, PrivacyCallsRules.persist(enabled))
    }

    fun rejectIncomingCalls(): Boolean = PrivacyCallsRules.parse(get(PrivacyCallsRules.KEY))
