...
class LocalStore(context: Context) : SQLiteOpenHelper(context, "rope-local.db", null, VERSION) {
    ...
    fun saveAutoDownload(prefs: AutoDownloadPrefs) {
        put("auto_dl", AutoDownloadRules.toJson(prefs))
    }

    fun autoDownload(): AutoDownloadPrefs = AutoDownloadRules.parse(get("auto_dl"))
