...
class LocalStore(context: Context) : SQLiteOpenHelper(context, "rope-local.db", null, VERSION) {
    ...
    fun saveVoiceEarpiece(enabled: Boolean) {
        put(VoiceEarRules.KV, if (enabled) "1" else "0")
    }

    fun voiceEarpieceEnabled(): Boolean = VoiceEarRules.earpieceFromKv(get(VoiceEarRules.KV))
